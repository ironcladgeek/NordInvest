...
class CrewAIAgentFactory:
    ...
    def create_technical_analysis_agent(self, tools: list = None) -> Agent:
        ...
        return Agent(
            ...
            backstory=(
                ...
                "chart patterns, moving averages, momentum indicators, and "
                "volume analysis. "
                "You receive pre-calculated technical indicators from the "
                "Calculate Technical Indicators tool "
                "and must extract ALL available indicator values into your "
                "structured output. "
                "The tool provides comprehensive data including: RSI, MACD "
                "(line, signal, histogram), "
                "Bollinger Bands (upper, middle, lower), ATR, moving averages "
                "(SMA 20/50, EMA 12/26, WMA 14), "
                "ADX with directional indicators (+DI, -DI), Stochastic "
                "Oscillator (%K, %D), "
                "and Ichimoku Cloud components (Tenkan, Kijun, Senkou A/B, "
                "Chikou). "
                "CRITICAL: You must populate ALL indicator fields in your output "
                "schema with the values "
                "from the tool - do not omit any available indicators. "
                "After extracting all values, interpret what these signals mean: "
                "identify trend strength, potential reversals, overbought/"
                "oversold conditions, "
                "and support/resistance levels. Your analysis incorporates "
                "multiple timeframes "
    ...
    def create_sentiment_analysis_agent(self, tools: list = None) -> Agent:
        ...
        return Agent(
            ...
            backstory=(
                "You are a skilled sentiment analyst with expertise in natural "
                "language processing, "
                "news interpretation, and market psychology. "
                "You analyze news article titles and summaries to determine "
                "sentiment (positive/negative/neutral) "
                "and assign sentiment scores. You classify events by type "
                "(earnings, regulatory, product launches, "
                "competitive, macro) and assess their importance and potential "
                "market impact. "
                "You understand how different types of news affect company "
                "valuations and stock prices, "
                "and can identify whether sentiment represents rational analysis "
                "or emotional overreaction. "
                "You excel at identifying key themes, catalysts, and sentiment "
                "trends from news coverage." + self.TOOL_FORMAT_INSTRUCTION
            ),
            tools=tools or [],
            llm=self.llm_client,
            verbose=False,
    ...
    def create_signal_synthesizer_agent(self) -> Agent:
        ...
        return Agent(
            ...
            backstory=(
                "You are a senior investment strategist responsible for "
                "synthesizing insights "
                "from multiple analysis perspectives. "
                "You combine technical, fundamental, and sentiment signals into "
                "coherent "
                "investment theses. You weigh conflicting signals, identify "
                "consensus areas, "
                "and provide clear recommendation rationale. "
                "You assess confidence levels based on signal agreement and data "
                "quality. "
                "You generate human-readable investment narratives that explain "
                "the investment case. "
                "\n"
                "IMPORTANT: You do NOT have access to any tools. All necessary "
                "analysis data "
                "(technical indicators, fundamental metrics, sentiment analysis) "
                "will be provided "
                "directly in your task description. Your job is to synthesize this "
                "pre-computed data "
                "into a structured JSON investment signal. Do NOT attempt to fetch "
                "or calculate any data. "
                "Focus entirely on analyzing the provided results and generating "
    ...
    def create_technical_analysis_task(
        ...
        return Task(
            description=(
                ...
                f"Use the 'Calculate Technical Indicators' tool with "
                f"ticker='{ticker}' to get "
                f"comprehensive technical data. The tool returns ALL available "
                f"indicators including:\n"
                f"- RSI (rsi)\n"
                ...
                f"- Ichimoku Cloud (ichimoku_tenkan, ichimoku_kijun, "
                f"ichimoku_senkou_a, ichimoku_senkou_b, ichimoku_chikou)\n"
                f"\n"
                f"CRITICAL: You MUST extract ALL indicator values from the tool "
                f"output and populate "
                f"ALL corresponding fields in your TechnicalAnalysisOutput. "
                f"Do not omit any indicators - "
                ...
                f"1. Analyze the trend (is the price above/below key moving "
                f"averages?)\n"
                f"2. Assess momentum (what do RSI, MACD, and Stochastic values "
                f"indicate?)\n"
            ...
            agent=agent,
            expected_output=(
                "Complete TechnicalAnalysisOutput with ALL indicator fields "
                "populated (RSI, MACD components, "
                "Bollinger Bands, ATR, all moving averages, ADX components, "
                "Stochastic components, and "
                "Ichimoku Cloud components) plus trend assessment, momentum "
    ...
    def create_fundamental_analysis_task(
        ...
        return Task(
            description=(
                ...
                f"1. Analyst Consensus - Professional analyst recommendations "
                f"(strong buy/buy/hold/sell/strong sell)\n"
                f"2. News Sentiment - Aggregated sentiment from news coverage "
                f"(positive/negative/neutral %)\n"
                f"3. Price Momentum - 30-day price change and trend "
                f"direction\n"
                f"4. Financial Metrics (from Yahoo Finance):\n"
                f"   - Valuation: trailing_pe, forward_pe, price_to_book, "
                f"price_to_sales, peg_ratio, enterprise_to_ebitda\n"
                f"   - Profitability: gross_margin, operating_margin, "
                f"profit_margin, return_on_equity, return_on_assets\n"
                f"   - Financial Health: debt_to_equity, current_ratio, "
                f"quick_ratio, free_cashflow\n"
                f"   - Growth: revenue_growth, earnings_growth\n"
                f"\n"
                f"CRITICAL: Extract and include the financial metrics in your "
                f"output:\n"
                ...
                f"- ev_ebitda: Use enterprise_to_ebitda from valuation "
                f"metrics\n"
                f"- profit_margin: Use profit_margin from profitability "
                f"metrics\n"
                f"- operating_margin: Use operating_margin from profitability "
                f"metrics\n"
                ...
                f"- debt_to_equity: Use debt_to_equity from financial_health "
                f"metrics\n"
                f"- current_ratio: Use current_ratio from financial_health "
                f"metrics\n"
                ...
                f"1. Valuation attractiveness (P/E, P/B, PEG - lower often "
                f"better)\n"
                ...
                f"\n"
                f"Do NOT make up or hallucinate financial metrics. Use ONLY the "
                f"data from the tool.\n"
                f"If specific metrics are not available from the tool, set them "
                f"to null in your output.\n"
                f"\n"
                f"Provide a fundamental score (0-100) based on the real data "
                f"fetched."
            ),
            agent=agent,
            expected_output=(
                "Fundamental analysis with financial metrics (P/E, margins, "
                "ROE, debt ratios, growth), "
                "analyst consensus interpretation, valuation assessment, and "
    ...
    def create_sentiment_analysis_task(
        ...
        return Task(
            description=(
                ...
                f"\n"
                f"Use the 'Analyze Sentiment' tool with ticker='{ticker}' to "
                f"fetch recent news articles.\n"
                f"\n"
                ...
                f"3. Classify the event type (earnings, product launch, "
                f"partnership, regulatory, competitive, etc.)\n"
                f"4. Assess importance/impact (0-100)\n"
                f"\n"
                f"Then synthesize the analysis:\n"
                f"1. Calculate overall sentiment distribution (% positive/"
                f"negative/neutral)\n"
                f"2. Identify major themes and patterns\n"
                f"3. Highlight key catalysts or events\n"
                f"4. Assess whether sentiment aligns with or contradicts "
                f"fundamentals\n"
                f"5. Determine if there are any sentiment shifts or trends\n"
                f"6. Provide an overall sentiment score (0-100)\n"
                f"\n"
                f"Focus on quality analysis - sentiment classification requires "
                f"understanding context, "
                f"not just keyword matching."
            ),
            agent=agent,
            expected_output=(
                "Sentiment analysis with article-level sentiment scores, key "
                "themes, event classification, "
                "overall sentiment distribution, and final sentiment score "
                "(0-100)"
            ),
    ...
    def create_signal_synthesis_task(
        ...
        return Task(
            description=(
                ...
                f"✗ DO NOT: Generate explanations, markdown, or any text other "
                f"than the JSON\n"
                ...
                f"Generate ONLY a valid JSON object with this exact structure "
                ...
                f"- Calculate final_score with weights: fundamental 35%, "
                f"technical 35%, sentiment 30%\n"
                f"- Recommendation based on final_score: >80=strong_buy, "
                f"70-80=buy, 60-70=hold_bullish, "
                f"50-60=hold, 40-50=hold_bearish, 30-40=sell, "
                f"<30=strong_sell\n"
            ...
            agent=agent,
            expected_output=(
                "Valid JSON object matching the InvestmentSignal schema with "
